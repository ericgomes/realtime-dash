...
const { resolveTenant, domainAllowed } = require('./tenants');
...
async function handleIngest(req, res, tenantSlug) {
  ...
  const clampRate = (v) => (typeof v === 'number' && v > 0 && v <= 1) ? v : null;
  const declaredRate = clampRate(body.sample_rate);
  const cap = clampRate(tenant.sample_rate) || 1;
